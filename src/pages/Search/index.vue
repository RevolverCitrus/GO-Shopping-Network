...
export default {
    ...
    computed:{
        //mapGetters里面的写法：传递的是数组，因为getters里面是没有划分模块的
    ...
    methods:{
        // 向服务器发请求获取search模块的数据（根据参数的不同返会不同的数据进行展示）
        ...
        removeCategoryName(){
            ...
            //带给服务器的参数说明是可有可无，如果属性值为空还是会带给服务器
            // 但是如果把属性值设置为undefined，那当前这个字段就不会带给服务器
            ...
            this.getData();
            // 地址栏也需要修改，进行路由跳转
            // 利用路由跳转，跟新地址栏的参数信息，自己跳转自己
            // 本意只是删除query参数
            // 如果带有params参数还是要带着,例如带有关键字华为
        ...
        removeKeyword(){
            ...
            // 再一次发送请求，跟新数据
            this.getData();
            // 触发全局数据总线的clear函数，清除Header组件的keyword关键字
            this.$bus.$emit('clear');
            // 进行路由的跳转
            // 清除query参数，即关键字
            if(this.$route.query){
    ...
    watch:{
        ...
        $route(newValue,oldValue){
            // 每次请求完毕应该把对应的1，2，3级分类的id置空，让其接收下一次的相应的id
            // 分类的名字和关键字不用清理：因为每一次路由发生变化的时候都会获得新的数据
            this.searchParams.category1Id='';
            this.searchParams.category2Id='';
            this.searchParams.category3Id='';            
            // 再次发起请求之前，先合并对象，整理带给服务器的参数
            Object.assign(this.searchParams,this.$route.query,this.$route.params);
            // 再次发起Ajax请求
            this.getData();
        }
