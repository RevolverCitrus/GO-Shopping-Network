  ...
  export default {
    ...
    methods:{
      ...
      changeActive(saleAttrValue,arr){
        // 遍历全部的售卖属性的属性值改变他们的isChecked属性改为0，那就没有高亮了
        arr.forEach(item=>{
          item.isChecked=0;
        });
        // 点击哪个售卖属性的属性值改为高亮
      ...
      changeSkuNum(event){
        ...
        // 如果用户输入的是非法的数字或者是小于1的数字
        if(isNaN(value)||value<1){
          this.skuNum=1;
        }else{
          // 正常大于1【但有可能不是整数，那就取整】，Meth.ceil取整
      ...
      async addShopCar(){
        ...
        // 当前这里是派发一个action，也向服务器发送请求，判断加入购物车是成功还是失败了，进行相应的操作
        // 下面这一行代码：调用了addOrUpdateShopCart函数，这个方法加入了async，返回的是一共Promise
        // 2.你需要知道这次的请求成功还是失败，如果成功进行路由跳转，如果失败返回给用户提示信息
        try{
          await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum});
          // 3.成功--进行路由的跳转
          // 4.在进行路由跳转的时候还需要将产品的信息传给下一级的路由组件

          // 下面这种手段路由跳转以及传递参数可以---但是路径会很难看
          // /addcartsuccess?skuInfo=%5Bobject%20Object%5D&skuNum=7
